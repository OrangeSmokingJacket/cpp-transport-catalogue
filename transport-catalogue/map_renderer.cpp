...
namespace renderer
{
	...
	{
		settings.line_width = lw;
	}
	void MapRenderer::SetStopRadius(double r)
	{
		settings.stop_radius = r;
	}
	void MapRenderer::SetStopLabelFontSize(int size)
	{
		settings.stop_label_font_size = size;
	}
	void MapRenderer::SetStopLabelOffset(double dx, double dy)
	{
		settings.stop_label_offset.x = dx;
		settings.stop_label_offset.y = dy;
	}
	void MapRenderer::SetStopLabelOffset(svg::Point p)
	{
		settings.stop_label_offset = p;
	}
	void MapRenderer::SetBusLabelFontSize(int size)
	{
		settings.bus_label_font_size = size;
	}
	void MapRenderer::SetBusLabelOffset(double dx, double dy)
	{
		settings.bus_label_offset.x = dx;
		settings.bus_label_offset.y = dy;
	}
	void MapRenderer::SetBusLabelOffset(svg::Point p)
	{
		settings.bus_label_offset = p;
	}
	void MapRenderer::SetUnderlayerColor(svg::Color color)
	{
		settings.underlayer_color = std::move(color);
	}
	void MapRenderer::SetUnderlayerWidth(double ulw)
	{
		settings.underlayer_width = ulw;
	}
	void MapRenderer::AddToColorPallete(svg::Color color)
	{
		settings.color_pallete.push_back(std::move(color));
	}
	...
	svg::Document MapRenderer::CreateCanvas(std::vector<Route*> routes)
	{
		...
		const SphereProjector projection{ coords.begin(), coords.end(), settings.width, settings.height, settings.padding };
	...
	const MapRendererSettings& MapRenderer::GetSettings() const
	{
		return settings;
	}
	...
	void MapRenderer::AddRouteNameToCanvas(svg::Document& canvas, Route* route, const SphereProjector& projection)
	{
		...
		route_name_begin.SetOffset(settings.bus_label_offset);
		route_name_begin.SetFontSize(settings.bus_label_font_size);
		...
		if (route->GetRouteType() == Route::Linear && *stops.begin() != *stops.rbegin())
		{
			...
			route_name_end.SetOffset(settings.bus_label_offset);
			route_name_end.SetFontSize(settings.bus_label_font_size);
	...
	svg::Color MapRenderer::GetColorFromPallete()
	{
		...
		current_pallete_index = (current_pallete_index + 1) % settings.color_pallete.size();
		return settings.color_pallete.at(prev_index);
	}
}